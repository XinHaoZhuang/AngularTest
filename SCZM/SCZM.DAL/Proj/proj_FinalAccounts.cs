...
{
    /// <summary>
    /// 数据访问类proj_FinalAccounts。
    /// </summary>
    public partial class proj_FinalAccounts
    {
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(SCZM.Model.Proj.proj_FinalAccounts model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into proj_FinalAccounts(");
        ...
        /// 更新一条数据
        /// </summary>
        public int Update(SCZM.Model.Proj.proj_FinalAccounts model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update proj_FinalAccounts set ");
        ...
        /// 得到一个对象实体
        /// </summary>
        public SCZM.Model.Proj.proj_FinalAccounts GetModel(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 ID,ContractId,FinalAccountsDate,FinalAccountsNat,Memo,FlagDel,OperaId,OperaName,OperaTime from proj_FinalAccounts ");
            strSql.Append(" where FlagDel=0 and ID=@ID ");
            SqlParameter[] parameters = {
                new SqlParameter("@ID", SqlDbType.Int,4)};
            parameters[0].Value = ID;

            SCZM.Model.Proj.proj_FinalAccounts model = new SCZM.Model.Proj.proj_FinalAccounts();
        ...
        /// </summary>
        public SCZM.Model.Proj.proj_FinalAccounts DataRowToModel(DataRow row)
        {
            SCZM.Model.Proj.proj_FinalAccounts model = new SCZM.Model.Proj.proj_FinalAccounts();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update proj_FinalAccounts set FlagDel=1 ");
        ...
        public DataSet GetList(string strWhere, int operaId)
        {
            ...
            strSql.Append("FROM proj_FinalAccounts a ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select a.FinalAccountsDate,a.FinalAccountsNat,a.Memo,a.OperaName,a.OperaTime,b.ID as ContractId,b.ContractCode,g.CompanyName,b.ContractDate,b.ContractNat,c.ProjName,d.CustName,e.PerName as SvcManagerName,f.PerName as ProjManagerName ");
            strSql.Append("FROM proj_FinalAccounts a ");
            strSql.Append("left join proj_Contract b on a.ContractId=b.ID ");
            strSql.Append("left join proj_Project c on b.ProjId=c.ID ");
            strSql.Append("left join base_Customer d on c.CustId=d.ID ");
