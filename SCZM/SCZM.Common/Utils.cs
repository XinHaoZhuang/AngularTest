...
namespace SCZM.Common
{
    public class Utils
    {
        ...
        {
            return Keys.ASSEMBLY_VERSION;
        }
        public static string GetVersionTime()
        {
            return Keys.ASSEMBLY_TIME;
        }
        ...
        {
            if ((expression == null) || (expression.Length > 20))
                return defValue;
        ...
        /// <returns></returns>
        public static string GetUrlDomain(string urlPage)
        {
            return Regex.Match(urlPage, @"(?<=://)[a-zA-Z\.0-9]+(?=\/)").Value.ToString();
        }
        #endregion
        ...
        /// <summary>
        /// DataRow[] 对象 转换为Json 字符串

        /// </summary>
        /// <param name="dt"></param>
        /// <returns></returns>
        public static string ToJson(DataRow[] dr, string[] columnName)
        {
            JavaScriptSerializer javaScriptSerializer = new JavaScriptSerializer();
            javaScriptSerializer.MaxJsonLength = Int32.MaxValue; //取得最大数值
            ...
            foreach (DataRow dataRow in dr)
            {
                ...
                {
                    if (Array.IndexOf<string>(columnName, dataColumn.ColumnName) != -1)
                    {
                        dictionary.Add(dataColumn.ColumnName, dataRow[dataColumn.ColumnName].ToString());
                    }
                }
