...
namespace SCZM.BLL.System
{
    ...
    {
        private readonly SCZM.DAL.System.sys_Post dal = new SCZM.DAL.System.sys_Post();
        public sys_Post()
        { }
        #region  基本方法
        /// <summary>
        ...
        /// </summary>
        public int Add(SCZM.Model.System.sys_Post model, out string message)
        {
            message = "新增成功！";
            if (dal.ExistsPostName(model.PostName))
            {
                message = "对不起，该岗位名称已存在！";
                return 0;
            }
        ...
        /// </summary>
        public bool Update(SCZM.Model.System.sys_Post model, out string message)
        {
            message = "修改成功！";
            string oldPostname = dal.GetModel(model.ID).PostName;
            if (oldPostname != model.PostName)
            {
                if (dal.ExistsPostName(model.PostName))
                {
                    message = "对不起，该岗位名称已存在！";
            ...
            else
            {
                if (oldPostname != model.PostName)
                {
                    if (dal.UpdatePostName(oldPostname, model.PostName) == false)
                    {
                        message = "修改成功,但更新审批流岗位名称失败！";
                        return true;
                    }
                }
                return true;
        ...
        {
            message = "删除成功！";
            if (dal.CheckPostUse(IDList) == 1)
            {
                message = "对不起，所选记录中存在已使用的岗位，不能删除！";
                return false;
            }
        ...
        /// </summary>
        public SCZM.Model.System.sys_Post GetModelByCache(int ID)
        {

            string CacheKey = "sys_PostModel-" + ID;
            ...
            return (SCZM.Model.System.sys_Post)objModel;
        }
        ...
        /// 获得数据列表
        /// </summary>
        public List<SCZM.Model.System.sys_Post> GetModelList(string strWhere)
        {
        ...
        /// 获得数据列表
        /// </summary>
        public List<SCZM.Model.System.sys_Post> DataTableToList(DataTable dt)
        {
            List<SCZM.Model.System.sys_Post> modelList = new List<SCZM.Model.System.sys_Post>();
            int rowsCount = dt.Rows.Count;
            if (rowsCount > 0)
            {
                SCZM.Model.System.sys_Post model;
