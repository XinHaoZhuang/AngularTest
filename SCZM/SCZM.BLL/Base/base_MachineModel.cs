...
{
    /// <summary>
    /// 业务逻辑类base_MachineModel 的摘要说明。
    /// </summary>
    public partial class base_MachineModel
    {
        private readonly SCZM.DAL.Base.base_MachineModel dal = new SCZM.DAL.Base.base_MachineModel();
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(SCZM.Model.Base.base_MachineModel model, out string message)
        {
            message = "保存成功！";

            if (dal.Exists("MachineModel", model.MachineModel, 0)) {
                message = "该机型已存在，保存失败！";
                return 0;
            }
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(SCZM.Model.Base.base_MachineModel model, out string message)
        {
            message = "保存成功！";
            if (dal.Exists("MachineModel", model.MachineModel, model.ID))
            {
                message = "该机型已存在，保存失败！";
                return false;
            }
