$(function () {
    ...
    $.swfUpLoadDefaults.property = {
        ...
        level: 2,//控制upload路径返回的级次
        photonameSuffix: ""//控制生成的hid_photo_name的值，默认3项数据，可在后面加第四项以区分第几个批量上传控件 王晓平 2016-08-06
    ...
    $.fn.InitSWFUpload = function (p) {
        ...
        var swfu = new SWFUpload({
            ...
            //button_text:  p.btntext, //上传按钮的文字
            button_text: '<span class="btnText">' + p.btntext + '</span>', //上传按钮的文字
            button_text_style: ".btnText{font-family:Microsoft YaHei;font-size:12px;line-height:28px;color:#FF0000;text-align:center;vertical-align:middle;}", //按钮样式
            
            button_text_top_padding: 3,
            button_window_mode: SWFUpload.WINDOW_MODE.WINDOW, //背景透明
            ...
            custom_settings: {
                ...
                "level": p.level,
                "photonameSuffix": p.photonameSuffix,
    ...
    try {
        ...
        if (jsonstr.status == '1') {
            ...
            if (this.customSettings.button_action == SWFUpload.BUTTON_ACTION.SELECT_FILE) {
                ...
                addImage($(this.customSettings.upload_target), jsonstr.path, jsonstr.thumb, this.customSettings.level, this.customSettings.photonameSuffix);
            }
            var progressObj = $(this.customSettings.upload_target).children(".upload-progress");
...
//======================================图片相册处理事件======================================
//添加图片相册
function addImage(targetObj, originalSrc, thumbSrc, level, photonameSuffix) {
    //插入到相册UL里面
    ...
    var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name" value="0|' + originalSrc + '|' + thumbSrc + photonameSuffix + '" />'
