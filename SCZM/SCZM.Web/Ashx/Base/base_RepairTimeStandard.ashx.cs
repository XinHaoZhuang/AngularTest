...
namespace SCZM.Web.Ashx.Base
{
    ...
    public class base_RepairTimeStandard : IHttpHandler, IReadOnlySessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "GetList": //获取工时标准列表
                    GetList(context, btn);
                    break;
                case "GetDetail": //获取工时标准明细
                    GetDetail(context, btn);
                    break;
                case "SaveData": //保存工时标准信息
                    SaveData(context, btn);
                    break;
                case "DelData": //删除工时标准信息
                    DelData(context, btn);
                    break;
            }
        }
