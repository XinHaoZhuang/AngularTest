...
namespace SCZM.Web.Ashx.Base
{
    ...
    /// base_FeeItem 的摘要说明
    /// <summary>
    public class base_FeeItem : IHttpHandler, IReadOnlySessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "GetList": //获取费用项目列表
                    GetList(context, btn);
                    break;
                case "GetDetail": //获取费用项目明细
                    GetDetail(context, btn);
                    break;
                case "SaveData": //保存费用项目信息
                    SaveData(context, btn);
                    break;
                case "DelData": //删除费用项目信息
                    DelData(context, btn);
                    break;
            }
        }
        ...
        private void GetList(HttpContext context, string btn)
        {
            ...
            try
            {
                ...
                SCZM.BLL.Base.base_FeeItem bll = new SCZM.BLL.Base.base_FeeItem();
        ...
        private void SaveData(HttpContext context, string btn)
        {
            ...
            string FeeItemName = RequestHelper.GetString("FeeItemName");
