...
namespace SCZM.Web.Ashx.Base
{
    ...
    /// base_PayType 的摘要说明
    /// <summary>
    public class base_PayType : IHttpHandler, IReadOnlySessionState
    {
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "GetList": //获取付款方式列表
                    GetList(context, btn);
                    break;
                case "GetDetail": //获取付款方式明细
                    GetDetail(context, btn);
                    break;
                case "SaveData": //保存付款方式信息
                    SaveData(context, btn);
                    break;
                case "DelData": //删除付款方式信息
                    DelData(context, btn);
                    break;
            }
        }
        ...
        private void GetList(HttpContext context, string btn)
        {
            ...
            try
            {
                ...
                SCZM.BLL.Base.base_PayType bll = new SCZM.BLL.Base.base_PayType();
        ...
        private void SaveData(HttpContext context, string btn)
        {
            ...
            string PayTypeName = RequestHelper.GetString("PayTypeName");
