...
namespace SCZM.DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        /// <summary>
        /// 执行SQL语句 向数据库里插入图像格式的字段(和上面情况类似的另一种实例)
        /// </summary>
        /// <param name="strSQL">SQL语句</param>
        ...
        /// 执行多条SQL语句，实现数据库事务
        /// </summary>
        /// <param name="SQLStringList">SQL命令行列表</param>
        /// <returns>影响的记录数</returns>
        ...
        /// <param name="content">参数内容,比如一个字段是格式复杂的文章，有特殊符号，可以通过这个方式添加</param>
        /// <returns>流水号</returns>
        public static object GetSingle(string SQLString, string content)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand cmd = new SqlCommand(SQLString, connection);
        ...
        /// <summary>
        /// 执行查询语句，返回DataSet 带时间限制
        /// </summary>
        /// <param name="SQLString">查询语句</param>
        /// <param name="Times">时间限制</param>
        /// <returns>DataSet</returns>
        ...
        /// 执行多条SQL语句，实现数据库事务。
        /// </summary>
        /// <param name="SQLStringList">SQL语句的CommandInfo LIST）</param>
        /// <returns>无</returns>
        ...
        #region 获取SQL语句
        /// <summary>
        /// 获取SQL语句  从SQL语句+Param
        /// </summary>
        /// <param name="command"></param>
        /// <returns></returns>
        public static string GetSqlFromPara(string strSql, SqlParameter[] parameters)
        {
            //string strSql ="";
            if (strSql != "")
            {
                foreach (SqlParameter parameter in parameters)
                {
                    ...
                    {
                        if (parameter.SqlDbType == SqlDbType.BigInt || parameter.SqlDbType == SqlDbType.Bit || parameter.SqlDbType == SqlDbType.Decimal || parameter.SqlDbType == SqlDbType.Float || parameter.SqlDbType == SqlDbType.Int || parameter.SqlDbType == SqlDbType.Money || parameter.SqlDbType == SqlDbType.SmallInt || parameter.SqlDbType == SqlDbType.TinyInt)
                        {
